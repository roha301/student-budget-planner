...
from planner.models import Expense, Category
from decimal import Decimal
from datetime import date

class OpenAICacheTestCase(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='cachetester', password='pass')
        self.token = Token.objects.create(user=self.user)
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token.key)
        cat = Category.objects.create(name='Food', user=self.user)
        Expense.objects.create(user=self.user, category=cat, amount=Decimal('4.50'), description='Coffee', date=date.today())
    ...
    def test_suggest_budget_cached(self, mock_create):
        ...
        mock_create.return_value = type('obj', (), { 'choices': [type('c', (), {'message': type('m', (), {'content': '{"monthly_income": 1000, "allocations": [], "notes": "ok"}'})()})]})()
        url = reverse('suggest_budget')
        # First call should invoke OpenAI
        resp1 = self.client.post(url, data={'months_of_history': 1}, format='json')
        self.assertIn(resp1.status_code, (200, 503))
        # Second call should hit the cache and not call the OpenAI API again
        resp2 = self.client.post(url, data={'months_of_history': 1}, format='json')
